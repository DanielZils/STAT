...
var SUMMENZIEL= "Sum(Case WHEN ServiceCode IN (196609,196611,196613,196614) THEN ANZAHL ELSE 0 END) AS 'Druck A4 SW',"
...
SUMMENZIEL = SUMMENZIEL + "Sum(Case WHEN ServiceCode IN (262145,262147,262149,262150,262155) THEN ANZAHL ELSE 0 END) AS 'Druck A4 C',"
SUMMENZIEL = SUMMENZIEL + "Sum(Case WHEN ServiceCode IN (262146,262152,262153,262154) THEN ANZAHL ELSE 0 END) AS 'Druck A3 C',"

SUMMENZIEL = SUMMENZIEL + "Sum(Case WHEN ServiceCode IN (65537,65542,65541,65547) THEN ANZAHL ELSE 0 END) AS 'Kopie A4 SW',"
SUMMENZIEL = SUMMENZIEL + "Sum(Case WHEN ServiceCode IN (65538,65543,65546,65545) THEN ANZAHL ELSE 0 END) AS 'Kopie A3 SW',"
SUMMENZIEL = SUMMENZIEL + "Sum(Case WHEN ServiceCode IN (131073,131075,131078,131077) THEN ANZAHL ELSE 0 END) AS 'Kopie A4 C',"
SUMMENZIEL = SUMMENZIEL + "Sum(Case WHEN ServiceCode IN (131074,131079,131082,131081) THEN ANZAHL ELSE 0 END) AS 'Kopie A3 C',"

SUMMENZIEL = SUMMENZIEL + "Sum(Case WHEN ServiceCode IN (393217,393219,393221,393222) THEN ANZAHL ELSE 0 END) AS 'Fax A4 SW',"
SUMMENZIEL = SUMMENZIEL + "Sum(Case WHEN ServiceCode IN (393218,393226,393223,393225) THEN ANZAHL ELSE 0 END) AS 'Fax A3 SW',"
...
SUMMENZIEL = SUMMENZIEL + "Sum(Case WHEN ServiceCode IN (327681,327685,327686,327692) THEN ANZAHL ELSE 0 END) AS 'Scan A4 SW',"
SUMMENZIEL = SUMMENZIEL + "Sum(Case WHEN ServiceCode IN (327682,327687,327689,327690) THEN ANZAHL ELSE 0 END) AS 'Scan A3 SW',"
SUMMENZIEL = SUMMENZIEL + "Sum(Case WHEN ServiceCode IN (589825,589827,589829,589830) THEN ANZAHL ELSE 0 END) AS 'Scan A4 C',"
SUMMENZIEL = SUMMENZIEL + "Sum(Case WHEN ServiceCode IN (589826,589831,589833,589834) THEN ANZAHL ELSE 0 END) AS 'Scan A3 C',"

SUMMENZIEL = SUMMENZIEL + "Sum(Case WHEN ServiceCode = 1048578 THEN ANZAHL ELSE 0 END) AS 'Duplex',"

SUMMENZIEL = SUMMENZIEL + "Sum(Case WHEN ServiceCode IN (196609,196611,196613,196614,65537,65542,65541,65547) THEN ANZAHL ELSE 0 END) AS 'A4 SW',"
SUMMENZIEL = SUMMENZIEL + "Sum(Case WHEN ServiceCode IN (196610,196612,196615,196617,196618,65538,65543,65546,65545) THEN ANZAHL ELSE 0 END) AS 'A3 SW',"
SUMMENZIEL = SUMMENZIEL + "Sum(Case WHEN ServiceCode IN (262145,262147,262149,262150,262155,131073,131075,131078,131077) THEN ANZAHL ELSE 0 END) AS 'A4 C',"
SUMMENZIEL = SUMMENZIEL + "Sum(Case WHEN ServiceCode IN (262146,262152,262153,262154,131074,131079,131082,131081) THEN ANZAHL ELSE 0 END) AS 'A3 C'"
...
// falls es keine letzte Abfrage gab (erster Aufruf)
   if (strSQL == "" )
   {
	 	...
	 	strSQL = "SELECT  Service_T.Name AS Service, Service_T.ServiceCode,  ServiceProvider_T.Name AS Name, ServiceProvider_T.MgmtData_Serial AS ["+ PppString.GetItem(1308) +"],"+ (!bIP ? "": "ISNULL(ServiceProvider_T.MgmtData_IPAddress,ServiceProvider_T.MgmtData_Hostname) AS ["+ PppString.GetItem(2451) +"],") +" ServiceProvider_T.MgmtData_Location AS ["+ PppString.GetItem(6232) +"], ServiceProvider_T.ID AS ["+ PppString.GetItem(1901) +"],"  + 
                      " CONVERT(varchar,ServiceUsage_T.UsageEnd,20) AS UsageEnd, " + 
                      " ServiceUsage_T.Cardinality AS Anzahl,  ServiceUsage_T.AmountPaid AS ["+ PppString.GetItem(1228) +"] " +
                      "FROM ServiceUsage_T  LEFT JOIN " +
                      "ServiceProvider_T ON ServiceUsage_T.ServiceProvider = ServiceProvider_T.ID  LEFT OUTER JOIN " +
